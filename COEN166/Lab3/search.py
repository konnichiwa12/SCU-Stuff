...
def breadthFirstSearch(problem):
    ...
    "*** YOUR CODE HERE ***"
    frontier = Queue()
    # item in frontier/queue is [current state, array containing path to current state]
    init = problem.getStartState()
    frontier.push([init , []])
    visited = []

    while (not frontier.isEmpty()):
        item = frontier.pop() #get next node
        visited.append(item[0]) #add current node to visited
        if (problem.goalTest(item[0])): #return if at goal state
            return item[1]
        next = problem.getActions(item[0]) #get the next paths of current state
        if (len(next) != 0): #if there are traversable paths
            for path in next:
                j = True #set flag for queue check
                a = problem.getResult(item[0], path) #next state
                b = item[1] + [path] #add next path to path to current state
                if ((not (a in visited))): #make sure we havent visited next node yet
                    for i in frontier.list: # (queue check) make sure next node isnt in queue
                        if (a == i[0]):
                            j = False
                else:
                    j = False
...
def depthFirstSearch(problem): 
    ...
    while (not frontier.isEmpty()):
        ...
        next = problem.getActions(item[0]) #get the next paths of current state
        if (len(next) != 0): #if there are traversable paths
            for path in reversed(next):
                j = True #set flag for stack check
                a = problem.getResult(item[0], path) #next state
                b = item[1] + [path] #add next path to path to current state
                if ((not (a in visited))): #make sure we havent visited next node yet
                    for i in frontier.list: # (queue check) make sure next node isnt in stack
                        if (a == i[0]):
                            j = False
                else:
                    j = False
