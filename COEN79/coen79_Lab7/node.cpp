...
namespace coen79_lab7
{
    ...
    void list_clear(node*& head) {//clear the list and return memeory to heap
        node* temp = head;
        while (temp != NULL){
            temp = head->getLink();
            delete head;
    ...
    void list_head_remove(node*& head) {////remove node pointed by head, return it to heap and set new head as the next element
        assert(head != NULL);
        node* temp = head;
        head = head->getLink();
        delete head;
